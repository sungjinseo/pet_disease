...
val_path = '/home/jupyter/workspace/eyes/val/'
...
class Fine_tunning:
    ...
    def training(self):
        ...
        save_folder = './model_saved/tf/' + data_name + '/' + self.model_name + '_' + str(self.epoch) + '/'
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        #check_point = ModelCheckpoint(save_folder + 'model-{epoch:03d}-{acc:03f}-{val_acc:03f}.h5', verbose=1, monitor='val_acc', save_best_only=True, mode='auto')
        check_point = ModelCheckpoint(save_folder + self.model_name+'.h5', verbose=1, monitor='val_acc', save_best_only=True, mode='auto')
